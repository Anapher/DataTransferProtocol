...
{
    internal class ObjectSerializer : IDynamicTypeSerializer
    {
        ...
        {
            var getTypeIdMethodInfo = typeof (Serializer).GetMethod("GetTypeID",
        ...
        public void GenerateReaderMethod(Type obtype, CodeGenContext ctx, ILGenerator il)
        {
            ...
            foreach (var kvp in map)
            {
                ...
                il.Emit(OpCodes.Ldarg_2);
                il.Emit(local.LocalIndex < 256 ? OpCodes.Ldloc_S : OpCodes.Ldloc, local);
                if (type.IsValueType)
